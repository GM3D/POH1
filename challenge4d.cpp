// challenge4d.cpp by GM3D ver 0.11
// same with challenge4.cpp, with time adjuster.
...
timeval t[2];
void adjuster(const int m, 
	      const float baseduration, const float tick,
	      const timeval t1, const timeval t0)
{
  float fduration = baseduration + m * tick
    - (t1.tv_sec - t0.tv_sec + 1e-6 * (t1.tv_usec - t0.tv_usec));
  if(fduration > 0.0){
    time_t sec = (int) fduration;
    long nsec = (int)(1e9 * (fduration - sec));
    timespec d;
    d.tv_sec = sec;
    d.tv_nsec = nsec;
    nanosleep(&d, NULL);
...
int main(int argc, char **argv)
{
  ...
  adjuster(c1.N, 0.01, 0.009576032806985737, t[1], t[0]);
  return 0;
}
